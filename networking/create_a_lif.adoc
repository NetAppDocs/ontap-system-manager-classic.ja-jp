...
* .10.1以降では、CLIコマンドにONTAP 9 `network interface` over RDMA構成のパラメータが含まれて `-rdma-protocols`います。ONTAP 9.12.1以降では、NFS over RDMA構成用のネットワークインターフェイスの作成がSystem Managerでサポートされています。詳細については、を参照してください xref:https://docs.netapp.com/us-en/ontap/nfs-rdma/configure-lifs-task.html[RDMA経由のNFS用のLIFの設定^]。
...
+
link:https://docs.netapp.com/us-en/ontap/san-admin/asa-iscsi-lif-fo-task.html["ASAプラットフォームでのiSCSI LIFフェイルオーバー"^]
